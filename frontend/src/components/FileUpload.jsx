...
export default function FileUpload({ onUpload, label = 'Upload de arquivo', accept = 'image/*,video/*' }) {
  ...
  return (
    <div>
      ...
      <div className="relative">
        <input type="file" accept={accept} onChange={handleFile} className="hidden" id={`upload-${label}`} />
        <label htmlFor={`upload-${label}`} className={`flex items-center justify-center gap-2 w-full py-3 border-2 border-dashed border-gray-200 rounded-xl text-sm cursor-pointer transition ${uploading ? 'opacity-50' : 'hover:border-tauste-orange hover:bg-tauste-orange/5'}`}>
          {uploading ? (
      ...
      {preview && <img src={preview} alt="" className="w-full h-24 object-cover rounded-lg mt-2" />}
    </div>
  )
}
