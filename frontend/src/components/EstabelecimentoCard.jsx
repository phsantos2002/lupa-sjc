  ...
  return (
    <div className="bg-white rounded-2xl overflow-hidden border border-gray-100 card-hover shadow-sm">
      ...
      <Link to={`/estabelecimento/${est.slug}`} className="block relative h-32 bg-tauste-blue overflow-hidden">
        ...
        {/* Status */}
        <span className={`absolute top-2.5 left-2.5 px-2 py-0.5 text-[10px] font-bold rounded-full ${est.aberto_agora ? 'bg-[#25D366] text-white' : 'bg-gray-800/80 text-gray-300'}`}>
      ...
      <div className="p-3">
        ...
        <div className="flex gap-2">
          {whatsLink ? (
            <>
              <a href={whatsLink} target="_blank" rel="noopener" onClick={e => e.stopPropagation()} className="flex-1 py-2 bg-[#25D366] text-white text-[11px] font-bold rounded-lg text-center flex items-center justify-center gap-1 hover:bg-green-600 transition">
