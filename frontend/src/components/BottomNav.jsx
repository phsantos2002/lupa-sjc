  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-50">
      <div className="max-w-5xl mx-auto flex justify-around items-center h-14">
        {items.map(item => (
          <NavLink key={item.to} to={item.to} end={item.to === '/'} className={({ isActive }) => `flex flex-col items-center gap-0.5 min-h-[44px] justify-center px-3 py-1 text-[11px] font-semibold transition-colors ${isActive ? 'text-tauste-orange' : 'text-gray-400'}`}>
            {item.icon}
